...
void YST93xSetMode(uint8_t mode) {
  switch (mode) {
    ...
    case YST93x_BAND_AM:
      ...
      bitWrite(pll_in1[0], IN10_R1, 0); // Reference frequency = 10kHz
      bitWrite(pll_in1[0], IN10_R2, 0); // Reference frequency = 10kHz
...
void loop() {
  if (digitalRead(up) == HIGH) {
    if (mode == 0) {
      ...
      FMFrequency += steppp;
      if(FMFrequency>1080)FMFrequency=800;
      tuned = YST93xTune(FMFrequency);
      display.showNumberDec(FMFrequency);
    } else if (mode == 2) {
      if(AMFrequency>180)AMFrequency=53;
      AMFrequency += steppp;
      tuned = YST93xTune(AMFrequency);
      display.showNumberDec(AMFrequency*10);
    }
    ...
    if (mode == 0) {
      ...
      FMFrequency -= steppp;
      if(FMFrequency<800)FMFrequency=1080;
      tuned = YST93xTune(FMFrequency);
      display.showNumberDec(FMFrequency);
    } else if (mode == 2) {
      AMFrequency -= steppp;
      if(AMFrequency<53)AMFrequency=180;
      tuned = YST93xTune(AMFrequency);
      display.showNumberDec(AMFrequency*10);
